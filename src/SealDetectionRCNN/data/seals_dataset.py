...
class SealsBboxDataset:
    """Bounding box dataset loader for the NOAA seals data based on the original
       VOTT / CNTK Faster RCNN format
    ...
    * :obj:`difficult.dtype == numpy.bool`
    ...
    def __init__(self, root):
        ...
        os.chdir(root)
        ext = '*.bboxes.tsv'
        self.impaths = sorted([fi[:-len(ext) + 1] for fi in glob.glob(ext)])
        print('Found {} images'.format(len(self.impaths)))
        self.image_ids = list(range(len(self.impaths)))

        self.img_ext = '_COLOR-8-BIT.JPG'
        self.ir_ext = '_THERM-REG-16BIT.PNG'
        ...
        for image_id, impath in enumerate(self.impaths):
            with open(impath + '.bboxes.labels.tsv', 'rt') as labelfile:
            ...
            bbox_coords = np.loadtxt(impath + '.bboxes.tsv', dtype=np.int32)
            ...
            width,height = PIL.Image.open(img_file + self.img_ext).size
    ...
    def validate_bboxes(self):
        ...
        try:
            ...
            for idx in tqdm(range(len(self.image_ids))):
                img_file = os.path.join(self.root, self.impaths[idx] + self.img_ext)
    ...
    def get_example(self, i):
        ...
        """
        if opt.ir_color_mode == 'color_only':
            img_file = os.path.join(self.root, self.impaths[i] + self.img_ext)
            img = read_image(img_file, color=True)
        elif opt.ir_color_mode == 'ir_only':
            img_file = os.path.join(self.root, self.impaths[i] + self.ir_ext)
            img = read_image(img_file, color=True)
        elif opt.ir_color_mode == 'ir_as_red':
            img_file = os.path.join(self.root, self.impaths[i] + self.img_ext)
            img = read_image(img_file, color=True)
            img_file_ir = os.path.join(self.root, self.impaths[i] + self.ir_ext)
            img_ir = read_image(img_file_ir)
            img[0, ...] = img_ir[0, ...]
        elif opt.ir_color_mode == 'four_channel':
            img = np.stack([img, img_ir], axis=0)
