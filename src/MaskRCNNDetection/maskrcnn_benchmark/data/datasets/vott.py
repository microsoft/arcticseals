...
class VOTTDataset(data.Dataset):
    ...
    * :obj:`difficult.dtype == numpy.bool`
    ...
    """
    def __init__(self, root, class_names = ['___background___'], transforms=None):
        self.root = root
        ...
        self.bboxes = [[] for _ in self.impaths]
        self.labels = [[] for _ in self.impaths]
        self.class_names = class_names
        empty_images = []
        for image_id, impath in enumerate(self.impaths):
            with open(os.path.splitext(impath)[0] + '.bboxes.labels.tsv', 'rt') as labelfile:
                bbox_labels = [ll.replace('rest', 'plastic') for ll in labelfile.read().splitlines()]
        ...
        print("Reading the width and height of all images...")
        self.img_dims = [PIL.Image.open(self.impaths[i]).size for i in self.image_ids]
        os.chdir(old_dir)
    ...
    def get_groundtruth(self, i):
        boxes = self.bboxes[i]
        boxes = torch.as_tensor(np.array(boxes)[:,[1,0,3,2]]).reshape(-1, 4)  # guard against no boxes
        target = BoxList(boxes, (self.get_img_info(i)['width'], self.get_img_info(i)['height']), mode="xyxy")

        classes = self.labels[i]
        target.add_field("labels", torch.tensor(classes))
        target.add_field("difficult", torch.tensor([False] * len(classes)))

        target = target.clip_to_image(remove_empty=True)
        return target

    def map_class_id_to_class_name(self, class_id):
        return self.get_class_names()[class_id]
    ...
    def get_img_info(self, i):
        ...
        # image from disk
        return {"height": self.img_dims[i][1], "width": self.img_dims[i][0]}
    ...
    def validate_bboxes(self):
        ...
        try:
            ...
            for idx in tqdm(range(len(self.image_ids))):
                ...
                width, height = self.img_dims[idx]
