...
Detection code for Faster RCNN and Mask-RCNN detection. Compared to the original code, we added support for
datasets in the VOTT format. In order to add your dataset, first add a link in the ./datasets directory, e.g.
    datasets/seals_large_IR_N/train
    datasets/seals_large_IR_N/test
Afterward configure `maskrcnn_benchmark/config/paths_catalog.py` to point to these location. The paths are relative to the
./datasets directory. It should look like:
    "vott_seals_large_IR_N_train": (
        "seals_large_IR_N/train",
    ),
    "vott_seals_large_IR_N_test": (
        "seals_large_IR_N/test",
    ),
Then adapt and run the `run_seals_IR_training.sh` training script to your need.
