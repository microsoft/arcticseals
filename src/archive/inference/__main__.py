...
def classifyThumbnails(folder):
    ...
    sys.path.append('../ir-hotspot-rfc')
    import hotspot_classifier as hc

    data_directory = os.path.join(folder, '..', '..', 'data', 'images')
    data_file = os.path.join(folder, '..', '..', 'data', 'testOut.csv')
    model_file = os.path.join(folder, '..','ir-hotspot-rfc','pca_rfc_model_20180725_154906.p')
    output_file = os.path.join(folder, '..', '..', 'data', 'irClassOut.csv')

    hc.classify_data(data_file, data_directory, model_file, output_file)
